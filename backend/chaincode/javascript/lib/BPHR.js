...
class BPHR extends Contract {
    ...
    async CreateReward(ctx, id, owner, date, item, outlet, value, isVerified) {
        ...
        if (!UniversityExists) {
            throw new Error('Only registered university users can create rewards.');
        }
    ...
    async ListRewards(ctx, owner){
        ...
        const universityExists = await this.UniversityExists(owner);
        if (!universityExists){
            throw new Error(`The user ${owner} is not a registered university and does not have any listed rewards.`);
        }

        // Filter all assets with "Reward": "yes" and "Owner" : owner
        // Store resulting rewards in the global reward array
        rewards = allAssets.filter(asset => asset.Reward === 'yes' && asset.Owner === owner).map(reward => JSON.stringify(reward));
    ...
    async VerifyPurchase(ctx, id, owner, outlet) {
        ...
        if (asset.Owner !== owner) {
            throw new Error(`Asset with ID ${id} does not belong to user ${owner}.`);
        }
        ...
        if (asset.Outlet !== outlet){
            throw new Error(`Outlet with ID ${outlet} is not the same as the one in the registered asset, which is ${asset.Outlet}.`);
        }
    ...
    async CheckEligibility(ctx, userID) {
        ...
        for (let i = 0; i < assets.length - 1; i++) {
            ...
            const currentDate = new Date(`${currentDateParts[2]}-${currentDateParts[1]}-${currentDateParts[0]}`);
            const nextDate = new Date(`${nextDateParts[2]}-${nextDateParts[1]}-${nextDateParts[0]}`);
    ...
    async TransferReward(ctx, userID, rewardID) {
        ...
        if(!universityExists){
            throw new Error(`The selected reward cannot be transferred as the owner ${reward.Owner} is not a registered university.`);
        }
